...
@section('content')
    ...
    <script>
        let countryRender = {
            ...
            addLanguage: function () {
                $('.add-language').click((e) => {
                    ...
                    this.createAndAppendOuterDiv(labelText, e)
            ...
            createAndAppendOuterDiv: function (labelText, e) {
                ...
                let span2 = $('<span>').addClass('flag-icon flag-icon-us inside-input').css('width', '30px');
                ...
                if (labelText === 'Description') {
                    ...
                    outerDiv.append(span1, textarea, span2, newSelect, button);
                } else {
                    let input = $('<input>')
                    ...
                    outerDiv.append(span1, input, span2, newSelect, button);
                }
        ...
        $('.class-field-select').change(function () {
            let arraySelect = ['on_hold_status_option', 'escalated_status_option'];

            // The element with class .reason-container exists
            // Create the main container
            var container = $('<div>', {class: 'input-group input-group-lg reason-container mt-3'});

            // Add the span element
            container.append($('<span>', {class: 'input-group-text', text: 'Reason'}));

            // Create the select element
            var select = $('<select>', {
                name: 'input[class-reason]',
                class: 'bulk-select class-reason-select',
                required: ''
            });

            if ($(this).val() === 'pending_status_option' || $(this).val() === 'on_hold_status_option' || $(this).val() === 'escalated_status_option') {
                $('.reason-container').remove()
                if (!$('.reason-container').length) {
                    // Add the default option
                    select.append($('<option>', {value: '', disabled: '', selected: '', text: 'Choose Reason'}));
                    select.append($('<option>', {value: 'awaiting_info', text: 'Awaiting information'}));
                    select.append($('<option>', {value: 'dependencies', text: 'Dependencies'}));
                    select.append($('<option>', {value: '3th_part_involvement', text: 'Third-party involvement'}));
                    select.append($('<option>', {value: 'resource_constraints', text: 'Resource constraints'}));
                    select.append($('<option>', {value: 'prioritization', text: 'Prioritization'}));
                    select.append($('<option>', {value: 'investigation_required', text: 'Investigation required'}));
                    select.append($('<option>', {value: 'pending_approval', text: 'Pending approval'}));
                    select.append($('<option>', {
                        value: 'process_policy_related_delay',
                        text: 'Process or policy-related delays'
                    }));
            ...
            } else if ($(this).val() === 'rejected_status_option') {
                $('.reason-container').remove()

                if (!$('.reason-container').length) {
                    // Add the default option
                    select.append($('<option>', {value: '', disabled: '', selected: '', text: 'Choose Reason'}));
                    select.append($('<option>', {value: 'insufficient_info', text: 'Insufficient information'}));
                    select.append($('<option>', {
                        value: 'non_compliance',
                        text: 'Non-compliance with policies or guidelines'
                    }));
                    select.append($('<option>', {value: 'lack_of_authorization', text: 'Lack of authorization'}));
                    select.append($('<option>', {value: 'resource_constraints', text: 'Resource constraints'}));
                    select.append($('<option>', {value: 'duplication_redundancy', text: 'Duplication or redundancy'}));
                    select.append($('<option>', {
                        value: 'contradiction',
                        text: 'Contradiction with objectives or priorities'
                    }));
                    select.append($('<option>', {
                        value: 'impractical_demands',
                        text: 'Unreasonable or impractical demands'
                    }));
                    select.append($('<option>', {value: 'safety_concerns', text: 'Safety or security concerns'}));
                    select.append($('<option>', {
                        value: 'non_compliance',
                        text: 'Non-compliance with eligibility criteria'
                    }));
                    select.append($('<option>', {
                        value: 'process_policy_related_delay',
                        text: 'Process or policy-related delays'
