...
@section('content')
    ...
    <script>
        let countryRender = {
            ...
            showElements: function (itemName) {
                // HIDE others
                this.hideElements();
                $(".form-control").prop("required", false)
                // SHOW description and rest of inputs
                $('.add-container-' + itemName).removeClass('d-none')
                $('.buttons-' + itemName).removeClass('d-none')
                $('.description-' + itemName).removeClass('d-none')
                $(".form-control:visible").prop("required", true);

            },

            hideElements: function () {
                // HIDE description and rest of inputs
                $('[class^="add-container-"]').addClass('d-none')
                $('[class^="buttons-"]').addClass('d-none')
                $('[class^="description-"]').addClass('d-none')
            },
            ...
            addLanguage: function (item) {
                $('.add-language').unbind('click')
                $('.add-language').on('click', (e) => {
                    e.preventDefault()
        ...
        $('.class-field-select').change((e) => {
            $(".form-control:not(.unchanged)").val('');
            if ($(e.currentTarget).val() !== '') {
                countryRender.showElements($(e.currentTarget).val());
                countryRender.initializeSelect2($('.countrySelect'), countryRender.countries);
                countryRender.addLanguage(e);
            } else {
                countryRender.hideElements()
            }
